// Epidemic.java
/**
 * Program to begin modelling epidemics
 * @author Logan Zweifel
 * @version 4/7/2021
 * Model Solution for MP8 used as a base template here
...
class MyScanner {
    ...
    // methods that we wish we could inherit from Scanner
    ...
    /** get the next name from the scanner or complain if missing
...
/** Class for extra utility/accuracy for floating points that
 *  the normal Math class does not
 */
class MathUtil {
	/** Helper method for floating point comparison with epsilon of .00001
	 *  @param x first double
	 *  @param y second double
	 *  both params are floating points to be compared
	 *  @return negative if x < y, 0 if equal, positive if x > y
	 */
	public static int compare(double x, double y) {
		// establish fixed epsilon
...
class Simulator {
	...
	/** Schedule all persons activities based on their schedule
	 *  @param duration
	 *  @return nothing because it's a void function
	 */
	public static void scheduleActivities(double duration) {
...
class Place {
	...
	public int contageous = 0; // how many infectious people are here
	 ...
	 *  Note: when an infectious person arrives, always call contageous(+1)
...
class PlaceKind {
    ...
    public PlaceKind( MyScanner in ) {
        ...
        // force transmissivity to be positive or zero
        transmissivity = Check.nonNeg( transmissivity, 0.0F, 
            ()-> this.describe() + ": negative transmissivity?"
        );

	    sigma = Math.log( (scatter + median) / median);
		allPlaceKinds.add( this ); // include this in the list of all placeKinds
    }
...
class Role {
	...
	private PlaceKind homePlaceKind = null;
    ...
    public Role( MyScanner in ) {
	    ...
	    while (hasNext && !in.tryNextLiteral( MyScanner.semicolon )) {
	        ...
	        if (duplicated) {
				...
				PlaceSchedule placeSched = new PlaceSchedule(pk, s);
				for (PlaceSchedule ps : placeKinds) {
					Schedule sch = ps.schedule;
					if (!(MathUtil.compare(s.startTime, sch.startTime) < 0
						|| MathUtil.compare(s.startTime, sch.endTime) > 0)
						&& (MathUtil.compare(s.endTime, sch.startTime) < 0
						|| MathUtil.compare(s.endTime, sch.endTime) > 0)) {
							Error.warn("Overlapping schedules exist");
						}
				}
				placeKinds.add(placeSched);
    ...
    public static void populateRoles( int population, int infected ) {
	    ...
	    for (Role r: allRoles) {
	        ...
	        for (int i = 0; i < r.number; i++) {
		        ...
		        r.homePlaceKind.populate(p, null);
...
class Person {
    ...
    Place location;            //initialized by emplace
    ...
    /** Associate this person to a particular place and vise versa
     *  @param p -- the place
     *  @param s -- the associated schedule
     */
    public void emplace( Place p, Schedule s ) {
		if (s != null) {
			...
			location.arrive(this);  //tell the location about the new occupant
		}
    }
    ...
    public void infect( double now ) {
	    if ((diseaseState == DiseaseStates.uninfected) 
			&& (infectMeTime == now)) { //can't do re-infection
	...
	/** Schedule a person's day based on their schedules
	 *  @param day -- day event is supposed to be schedules
	 *  @return -- nothing
	 */
	public void scheduleDay(int day) {
		for (PlaceSchedule ps: places) {
			Simulator.schedule(day * Time.day + ps.schedule.startTime,
				(double t)-> move(t)); //move to scheduled place
			Simulator.schedule(day * Time.day + ps.schedule.endTime,
				(double t)-> move(t)); //return after previous move
		}
	}

	/** Move's a person from current location to new one
	 *  @param time -- time moving of person should occur
	 *  @return -- nothing
	 */
	public void move(double time) {
		if (diseaseState != DiseaseStates.bedridden //don't move if bedridden
			&& diseaseState != DiseaseStates.dead) {//the dead don't move
				Place newPlace = getPlace(time);
				if (diseaseState != DiseaseStates.uninfected
					&& diseaseState != DiseaseStates.recovered) {
						location.contageous(time, -1);
						newPlace.contageous(time, 1);
					}
				location.depart(this);
				newPlace.arrive(this);
			}
	}

	/** Method to help find where person should go given by current time
	 * @param time -- current time
	 * @return -- which place person should go to
	 */
	private Place getPlace(double time) {
		for (PlaceSchedule ps : places) {
			if (!(MathUtil.compare(time, ps.schedule.startTime) < 0)
				&& !(MathUtil.compare(time, ps.schedule.endTime) > 0)) {
					return ps.place;
			}
		}
		return home;
	}
	/** Method starts people in their homes
	 *  Will only run during preparation of simulation
	 *  @return -- nothing
	 */
	public static void populateHome() {
...
public class Epidemic {
    ...
    private static void buildModel( MyScanner in ) {
		...
		//schedule events based on a persons schedule(s)
